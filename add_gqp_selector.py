...
# Lire gqp.html
with open('QE/Frontend/Entrepreneurs/Outils/GQP/gqp.html', 'r', encoding='utf-8') as f:
    content = f.read()

# Vérifier si le sélecteur existe déjà
if 'coach-entrepreneur-selector' in content:
    print("Le sélecteur existe déjà dans gqp.html!")
    exit(0)
...
# Trouver </head> et insérer le script avant
content = content.replace('</head>', anti_flash_script + '</head>')

# 2. Ajouter le backdrop et le sélecteur APRÈS <body>
selector_html = """
  ...
  <div id="coach-entrepreneur-selector">
    <!-- Titre de la page avec icône GQP -->
    <div class="page-identity">
      <div class="page-icon">
        <i class="fas fa-tasks"></i>
      </div>
      <div class="page-name">Gestion de Projet</div>
    </div>
    ...
    <div class="selector-subtitle">Choisissez l'entrepreneur dont vous souhaitez gérer les projets</div>
...
# Trouver <body et insérer après la balise complète
body_match = re.search(r'<body[^>]*>', content)
if body_match:
    insert_pos = body_match.end()
    content = content[:insert_pos] + selector_html + content[insert_pos:]

# 3. Ajouter le CSS complet AVANT </head> (après le script anti-flash)
css_selector = """  <style>
    /* ============================================
    ...
    .page-icon {
      ...
      box-shadow: 0 10px 30px rgba(168, 85, 247, 0.3), 0 0 40px rgba(168, 85, 247, 0.15);
      position: relative;
    }
    ...
    .page-icon::before {
      ...
      inset: -3px;
      border-radius: 23px;
      background: linear-gradient(135deg, rgba(168, 85, 247, 0.5), rgba(147, 51, 234, 0.3));
      z-index: -1;
    }
    ...
    .page-name {
      ...
      background: linear-gradient(135deg, #a855f7 0%, #9333ea 50%, #c084fc 100%);
...
# 4. Ajouter le script JavaScript AVANT </body>
js_script = """
<script>
  ...
  (function() {
    ...
    function selectEntrepreneur(username) {
      ...
      if (dropdownToggle) {
        ...
        // Recharger les projets pour l'entrepreneur sélectionné
...
with open('QE/Frontend/Entrepreneurs/Outils/GQP/gqp.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("Selecteur ajoute avec succes a gqp.html!")
print("- Icone: fa-tasks (violet)")
print("- CSS complet ajoute")
print("- Script JavaScript ajoute")
print("- Anti-flash script ajoute")
